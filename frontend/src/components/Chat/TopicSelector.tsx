...
export default function TopicSelector({ onTopicSelected }: TopicSelectorProps) {
  ...
  const selectedTopic = learning.state.topics.find(t => t.id === selectedTopicId);
    ...
    return (
        <div className="flex items-center justify-center h-full p-8">
          <div className="max-w-md text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-3">Welcome to LearnBase!</h2>
            <p className="text-gray-600 mb-6">
              You haven't created any topics yet. Start your learning journey by creating your first topic.
            </p>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-left">
              ...
              <p className="text-sm text-blue-800">
                Create a topic like <strong>JavaScript</strong>, <strong>Python</strong>, or any subject you want to learn!
              </p>
            </div>
          </div>
        </div>
        ...
        {step === 'topic' ? (
          <div className="space-y-6">
            <div className="text-center mb-8">
              ...
              <h2 className="text-3xl font-bold text-gray-900 mb-2">Let's Start Learning!</h2>
              <p className="text-gray-600">Choose a topic to begin your learning session</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Choose a Subtopic</h2>
                <p className="text-gray-600">Select a specific area within {selectedTopic?.name}</p>
              </div>
            </div>

            <div className="space-y-3">
              {['basic', 'intermediate', 'advanced'].map((level) => {
                const subtopics = selectedTopic?.subtopics.filter(
            ...
            <button
              onClick={() => onTopicSelected(parseInt(selectedTopicId!), selectedTopic?.name || '')}
              className="w-full bg-gray-100 border-2 border-gray-300 rounded-lg p-4 hover:bg-gray-200 transition text-center"
            >
              <span className="text-gray-700 font-medium">Start with general {selectedTopic?.name} topic</span>
            </button>
          </div>
        )}
      </div>
